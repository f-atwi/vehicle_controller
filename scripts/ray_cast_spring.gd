...
@export var damping_coefficient = 10000 # should be studied for its value
...
func _ready() -> void:
	...
	x_preloaded = spring_natural_length * x_preload_fraction
...
func _physics_process(delta: float) -> void:
	if is_colliding():
		var length: float = clamp(get_collision_point().distance_to(global_position) - wheel_radius, wheel_radius, spring_length)	
